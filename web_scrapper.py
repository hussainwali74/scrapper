    ...
    try:
        if ENV == "prod":
            ...
            driver = webdriver.Firefox(
                firefox_binary=firefox_binary_path, executable_path=GECKODRIVER_PATH, options=opts)
        else:
            driver = webdriver.Firefox(
                firefox_binary=firefox_binary_path, executable_path=GECKODRIVER_PATH)
        # get web page
        driver.get(page_url)
        time.sleep(6)
        ...
        total_cars = cars_found(
            main_soup, html_tag=cf_tag, attrs=cf_class_attrs)
        ...
        while cars_in_soup < total_cars:
            ...
            cars_in_soup = cars_count_in_soup(
                main_soup, html_tag=cs_tag, attrs=cs_class_attrs)
...
def get_all_car_page_links(url='https://www.hansenford.ca/inventory/used-vehicles/price-40000--/srp-page-1/',
    ...
    total_cars = cars_found(main_soup, html_tag='div', attrs={
                            "class": "sc-puHdH fiyXIl"})
    logging.info(f'Total cars on page: {total_cars}')
    current_loop_runs = 0
    while len(car_page_links) < total_cars:
        current_loop_runs += 1
        time.sleep(2)
        car_page_links.extend(get_car_page_links(
            main_soup, 'a', attrs={'class': 'sc-oTAMn cEnJNJ'}))
        try:
            next_page_soup = main_soup.find_all(
                'a', attrs={'class': 'sc-ptbNe hCFcIE'})[-1]
        except:
            next_page_soup = None
...
def regal_n_junct_north(url: str = 'https://www.regalmotorsltd.com/used/used-vehicle-inventory.html') -> list:
    ...
    def get_car_price(soap):
        try:
            price_html = soap.find(
                'span', attrs={'class': 'price-header', 'data-field': 'selected_price'})
    ...
    def get_car_info(soap, website):
        ...
        car_info = {"car_name": get_car_name(
            soap), "price": get_car_price(soap), "website": website}
    ...
    list_of_car_info = []
    all_div_table_row_lg = soup.find_all(
        'div', attrs={'class': 'table-row lg'})
    for div_table_row_lg in all_div_table_row_lg:
        all_div_padding = div_table_row_lg.find_all(
            'div', attrs={'class': 'padding'})
...
def woodridgeford(url: str) -> list:
    ...
    def get_car_mileage(soap):
        ...
        html_mileage_payment = soap.find(
            'span', attrs={'itemprop': 'mileageFromOdometer'})
        try:
    ...
    list_of_car_info = []
    all_class_outside_box = main_soup.find_all(
        'div', attrs={'class': 'col-xs-12 col-sm-12 col-md-12'})
...
def zarowny_n_westlock(url: str) -> list:
    ...
    def get_car_specs_raw(soap) -> list:
        ...
        for elem in range(0, len(car_specs_pieces), 2):
            car_specs.append(
                car_specs_pieces[elem] + car_specs_pieces[elem + 1])
    ...
    all_class_vehicle_card = main_soup.find_all(
        'li', attrs={'class': 'vehicle-card-used'})
...
def fourlane(url: str) -> list:
    ...
    def get_car_info(soap, website, car_page_url):
        ...
        if "aspenford.ca" in base_url:
            car_info["price"] = get_car_price(soap, html_tag='div', attrs={
                                              'id': 'fd-vehicle-price'})
        else:
            car_info["price"] = get_car_price(
                soap, html_tag='span', attrs={'class': 'price-large'})
    ...
    all_a_vehicle_media = main_soup.find_all(
        'a', href=True, attrs={'class': 'vehicle media'})
    for a_vehicle_media in all_a_vehicle_media:
        if a_vehicle_media.has_attr("href"):
            ...
            list_of_car_info.append(get_car_info(
                soup, base_url, base_url + url))
...
def marlborough(url: str) -> list:
    ...
    def get_car_specs_raw(soap):
        spec_list = []
        all_class_specification = soup.find_all(
            'ul', attrs={'class': 'specification col-sm-6'})
...
def universalford(url: str) -> list:
    ...
    def get_car_specs_raw(soap):
        spec_list = []
        all_class_specification = soap.find_all(
            'ul', attrs={'class': 'specification col-sm-6'})
        for class_specification in all_class_specification:
            for li in class_specification.find_all('li'):
                raw_li = li.getText().strip()
    ...
    def get_cars_from_single_page(main_soap, website):
        list_of_car_info = []  # car_info: dict
        # Get links of all car's information page
        all_class_vehicle_title = main_soap.find_all(
            'div', attrs={'class': 'vehicle-title'})
        for class_vehicle_title in all_class_vehicle_title:
            ...
            if class_title_link.has_attr("href"):
                ...
                list_of_car_info.append(
                    get_car_info(soup, website, car_page_url))
    ...
    while pagination_next.has_attr('href'):
        ...
        # Repeat
        pagination_next = main_soup.find(
            'a', attrs={'class': 'pagination-next'})
...
def camclarkfordairdrie(url: str) -> list:
    ...
    def get_car_info(soap, website, car_page_url):
        ...
        raw_car_specs = get_car_specs_raw(soap, html_tag='div', attrs={
                                          'class': 'detailed-specs__single-content'})
    ...
    if 'camclarkfordairdrie' in url:
        main_soup, driver = get_main_soup_n_driver(url, cf_tag='p',
                                                   cf_class_attrs={
                                                       "class": "srp__found-header wrapper"},
...
def collegefordlincoln(url: str) -> list:
    ...
    def get_car_info(soap, website, driver):
        ...
        if "royalford.ca" in website:
            raw_car_name = get_car_name(
                soap, html_tag='h2', attrs={'class': 'centered'})
        ...
        if "northlandford.mb.ca" in website:
            car_info["price"] = get_car_price(
                soap, html_tag='span', attrs={'class': 'fee-value'})
        else:
            # For "formomotors.com"
            price_html = soap.find(
                'span', attrs={'data-field': 'selected_price'})
            dealer_price = price_html.find(
                'div', attrs={'class': 'fee-row dealerPrice'})
            if dealer_price is None:
                car_info["price"] = get_car_price(soap, html_tag='span', attrs={
                                                  'data-field': 'selected_price'})
            else:
                car_info["price"] = get_car_price(soap, html_tag='div', attrs={
                                                  'class': 'fee-row dealerPrice'})
    ...
    list_of_car_info = []
    if "royalford.ca" in url or "hometownford.ca" in url:
        all_div_table_row_lg = main_soup.find_all(
            'div', attrs={'class': 'table-row lg md'})
    else:
        all_div_table_row_lg = main_soup.find_all(
            'div', attrs={'class': 'table-row lg'})
    for div_table_row_lg in all_div_table_row_lg:
...
def zenderford(url: str) -> list:
    ...
    def get_max_pages(main_soap):
        try:
            max_page_str = main_soap.find(
                'div', attrs={'class': 'pagination__numbers'}).span.getText()
        ...
        while current_page <= max_pages:
            ...
            for a_vehicle in all_a_vehicle:
                if a_vehicle.has_attr("href"):
                    ...
                    list_of_car_info.append(
                        get_car_info(soup, url, single_car_url))
...
def highriverford(url: str) -> list:
    ...
    def get_car_specs_raw(soap) -> list:
        ...
        for spec_key in ['miles', 'exteriorcolor', 'standardbody', 'drivetrain', 'transmission', 'enginedescription']:
            try:
                car_spec = soap.find(
                    'span', attrs={'class': f'spec-value spec-value-{spec_key}'})
    ...
    list_of_car_info = []
    all_vehicle_cards = main_soup.find_all(
        'div', attrs={'class': 'vehicle transformed'})
...
def hansenford(url: str) -> list:
    ...
    def get_car_specs_raw(soap):
        ...
        for i in range(0, len(car_specs), 2):
            spec_list.append(
                f'{car_specs[i].getText()} {car_specs[i + 1].getText()}')
        return spec_list
...
def windsorford(url: str) -> list:
    ...
    def get_all_car_page_links(url='https://www.windsorford.com/inventory/Used/?page=1',
        ...
        max_page_number = int(main_soup.find(
            'div', attrs={'class': 'inner-count pagination-max-page'}).getText())
        total_cars = cars_found(main_soup, html_tag='span', attrs={
                                "class": "search-vehicle-count"})
        logging.info(f'Total cars on page: {total_cars}')
        ...
        while curr_page_number <= max_page_number:
            ...
            car_page_links.extend(get_car_page_links(main_soup, 'div', attrs={'class': 'inventory-vehicle-header'},
                                                     base_url=base_url))
            url = url.replace(f"page={curr_page_number}",
            ...
            logging.info(
                f'Cars currently in car_page_link: {len(car_page_links)}')
            if current_loop_runs == FAIL_SAFE_RUNS:
                break
        return car_page_links
    ...
    def get_car_info(soap, website, car_page_link):
        ...
        raw_car_specs = get_car_specs_raw(
            soap, html_tag='div', attrs={'class': 'info-item'})
...
def northstarfordcarsandtrucks(url: str) -> list:
    ...
    def get_car_info(soap, website, car_page_link):
        ...
        raw_car_specs = get_car_specs_raw(soap, html_tag='li', attrs={
                                          'class': 'module-vehicleBulletsWidget'})
    ...
    list_of_car_info = []  # car_info: dict
    car_cards_soup_list = main_soup.find_all(
        'div', attrs={"class": "table-row xs"})
    for car_cards_soup in car_cards_soup_list:
        link_soup = car_cards_soup.find(
            'a', href=True, attrs={'class': 'button quick-view'})
...
def strathmoreford(url: str) -> list:
    ...
    total_cars = cars_found(main_soup, 'span', attrs={
                            'class': 'vehicle-count'})
    list_of_car_info = []  # car_info: dict
    ...
    while len(list_of_car_info) < total_cars:
        current_loop_runs += 1
        cars_card_soups = main_soup.find_all(
            'div', attrs={'class': 'hproduct auto ford has-sales-taxes'})
        for soup in cars_card_soups:
            list_of_car_info.append(get_car_info(soup, url, base_url))

        url = url.replace(f"start={start}",
                          f"start={start + cars_on_single_page}")
...
def revolutionford(url: str) -> list:
    ...
    while current_page <= max_pages:
        ...
        # Scrape all cars on a single page
        car_page_links = main_soup.find_all(
            'a', attrs={'class': 'rmv_txt_drctn'})
...
def novlanbros(url: str) -> list:
    ...
    def get_car_info(soap, website):
        ...
        raw_car_specs = soap.find(
            'div', attrs={'class': 'e-description description'}).getText().split('\n')
    ...
    list_of_car_info = []
    all_class_outside_box = main_soup.find_all(
        'div', attrs={'class': 'h-product hproduct vehicleModule'})
    for class_outside_box in all_class_outside_box:
...
def knightfordlincoln_n_capitalfordlincoln(url: str) -> list:
    ...
    def get_max_pages(main_soap):
        try:
            max_page_str = main_soap.find(
                'div', attrs={'class': 'pagination-state'}).getText()
    ...
    while current_page <= max_pages:
        ...
        all_car_cards = main_soup.find_all(
            'div', attrs={'class': 'result-wrap used-vehicle stat-image-link'})
...
def cypressmotors(url: str) -> list:
    ...
    page = driver.page_source
    main_soup = BeautifulSoup(page, 'html.parser')
    car_count_str = main_soup.find(
        'div', {'class': 'inventory-heading_count'}).getText()
    ...
    list_of_car_info = []
    all_vehicle_cards = main_soup.find_all(
        'div', attrs={"class": "inventory-item clearfix js-vehicle-item"})
    for vehicle_card in all_vehicle_cards:
        list_of_car_info.append(get_car_info(vehicle_card, url))
...
def birchwoodford(url: str) -> list:
    ...
    def get_all_car_page_links(url='https://www.birchwoodford.ca/vehicles/?results_page=1&condition=used&sort=price',
        ...
        while len(car_page_links) < total_cars:
            ...
            # Get car page links
            car_card_soups = main_soup.find_all(
                'div', {'class': 'phoenix4-vehiclecard'})
            ...
            # Go to next page
            url = url.replace(
                f'results_page={current_page}', f'results_page={current_page + 1}')
            current_page += 1
            page = requests.get(url)
            main_soup = BeautifulSoup(page.content, 'html.parser')
...
def merlinford(url: str) -> list:
    ...
    def get_car_info(soap, website):
        ...
        raw_car_specs = get_car_specs_raw(
            soap, html_tag='div', attrs={'class': 'w-4/5'})
...
def kelleherford(url: str) -> list:
    ...
    total_cars = cars_found(main_soup, html_tag='p', attrs={
                            'class': 'text-primary'})
    ...
    while len(list_of_car_info) < total_cars:
        ...
        logging.info(f'Scraping on Page: {current_page}')
        # Scrape all cars on a single page
        all_vehicle_card = main_soup.find_all(
            'div', {'class': 'card-1-inventory-item'})
        for vehicle_card_soup in all_vehicle_card:
            list_of_car_info.append(get_car_info(
                vehicle_card_soup, url, base_url))
...
def rainbowford(url: str) -> list:
    ...
    def get_car_info(soap, car_image_link, url, car_page_url):
        ...
        if car_price is None:
            car_price = get_car_price(
                soap, html_tag='td', attrs={'class': 'table-rw'})
    ...
    for car_card in car_cards_list:
        ...
        list_of_car_info.append(get_car_info(
            soup, car_image_link, url, single_car_url))
    ...
    while next_page_link_html is not None:
        ...
        # Go to next page
        main_soup, driver = main_soup_n_driver_scroll_down(
            next_page_link, driver)

        car_cards_list = main_soup.find_all(
        ...
        for car_card in car_cards_list:
            ...
            single_car_url = car_card.find(
                'a', {'class': 'rmv_txt_drctn'})["href"]
            soup, driver = main_soup_n_driver_scroll_down(
                single_car_url, driver)

            list_of_car_info.append(get_car_info(
                soup, car_image_link, url, single_car_url))

        # Check next page
        next_page_link_html = main_soup.find(
